...
export class BarChartDataBuilder {
    ...
    public build(): ChartData<"bar"> {
        ...
        return {
            labels,
            datasets: [...new Set<string>(this.dataPoints.map(d => d.seriesName!))].map(seriesName => (
                {
                    label: seriesName,
                    data: this.dataPoints.filter(d => d.seriesName === seriesName).map(d => d.value),
                    backgroundColor: this.dataPoints.filter(d => d.seriesName === seriesName).map(d => d.color!),
                    borderColor: this.dataPoints.filter(d => d.seriesName === seriesName).map(d => d.color!),
                    grouped: true
                }))
        };
    }
